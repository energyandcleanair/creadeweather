cfs.add_weather <- function(weather, weather_vars) {
  ...
  dates_files <- location_dates %>%
    rowwise() %>%
    mutate(
      fp = list(cfs.date_to_filepaths(date, dir_pbl, vars)),
  ...
  weather_new <- weather %>%
    ...
    mutate(weather = list(
      weather %>%
        left_join(
          data %>% select(c("date", intersect(names(data), weather_vars))), by = "date"
        )
    )) %>%
    select(-c(data))
...
cfs.folder_pbl <- function() {
  ...
  dir.create(dir_pbl_cfs, showWarnings = FALSE, recursive = TRUE)
...
cfs.processed_dates <- function() {
  dir_pbl <- cfs.folder_pbl()
  ds <- list.files(path = dir_pbl, full.names = FALSE, recursive = TRUE)
...
cfs.process_date <- function(date, cache_rs = NULL) {
  ...
  tryCatch({
    hours <- seq(
      lubridate::date(date) - lubridate::hours(14),
...
cfs.timezone_sf <- function() {
  ...
  if (!file.exists(fp)) {
    ...
    download_url <- paste0(
      ...
      "cultural/ne_10m_time_zones.zip"
    )
    download.file(download_url, fp_zip)
...
cfs.day_to_utc_hours <- function(date, timezone) {
  hour_start <- lubridate::floor_date(
    lubridate::with_tz(lubridate::force_tz(date, tz = timezone), tz = "UTC"),
    unit = "h"
  )
...
cfs.hour_to_filepath <- function(h, dir_cfs) {
  file.path(
    ...
    strftime(h, "%m"),
    strftime(h, "%d"),
    paste0("cfs_", strftime(h, "%Y%m%d_%H%M%S"), ".nc")
  )
}
...
cfs.date_to_filepaths <- function(d, dir_pbl, vars = c("pbl_min", "pbl_max", "pbl_mean")) {
  tibble(
    variable = vars,
    fp = file.path(dir_pbl, paste0(gsub("-", "", d), gsub("pbl", "", vars), ".nc"))
  )
}