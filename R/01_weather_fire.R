...
frp.add_frp <- function(weather,
  ...
  creatrajs::fire.download(date_from=min(w$date)-lubridate::hours(delay_hour+duration_hour),
                           date_to=max(w$date))
  print("Done")
  ...
  if(mode=="trajectory"){
    ...
    wtf <- creatrajs::fire.attach_to_trajs(wt, buffer_km=buffer_km)
    print("Done")
  ...
  result <- weather %>%
    left_join(wtf %>%
                tidyr::unnest(fires) %>%
                dplyr::select(station_id=location_id, date, fire_frp, fire_count) %>%
...
# }
# 
# frp.hysplit.trajs_at_dates <- function(dates, geometry, heights, duration_hour, met_type){
#   
...
#   paste(location_id, met_type, round(height), duration_hour, strftime(date, "%Y%m%d"),"RDS",
#         sep=".")
# }
...
#' #'
#' #' @examples
#' frp.trajs_at_dates <- function(dates, location_id, geometry, country, met_type, heights, duration_hour, ...){
#'   
#'   tryCatch({
#'     cache_folder <- utils.get_cache_folder("trajs")
#'     print(paste0("Calculating trajs for ", location_id,". Storing in ", cache_folder))
#'     filenames <- frp.cache.filename(location_id, country, met_type, heights, duration_hour, dates)
...
#'                                                   duration_hour=duration_hour, heights=heights.missing)
#'       
