...
prepare_input <- function(pollutants, deg=0.2){
  ...
  if(!dir.exists(input_folder)) dir.create(input_folder, recursive = T)
    ...
    # pollutants <-c('PM10') #'NO2','PM10','SO2','CO','O3') 
  ...
  # Then save
  filename <- paste('eea_meas_daily',paste(tolower(pollutants),collapse='_'),sub('\\.','',deg),'.RDS',sep='_')
